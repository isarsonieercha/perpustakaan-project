...
export class PenerbitService{
    ...
    deletePenerbit(id: string){
        ...
        let options = new RequestOptions({headers: headers});
        return this.http.delete(url + '/' + id).map(res => res.json()).catch(this.handleError);
    }
    ...
    let options = new RequestOptions({headers: headers});
    return this.http.post(url, penerbit, options).map(res => res.json()).catch(this.handleError);
    }
